...
Based on the graphical representation, we might infer that the number of accidents in states are correlated with the respective state populations. Although that may be an obvious starting point, we might want to check whether the proportion of drivers relative to the state populations varies significantly across states. If so, normalizing by state populations might misrepresent actual driving exposure, and using the number of licensed drivers or vehicle miles traveled could be a more appropriate denominator for accident rate comparisons across states. 
